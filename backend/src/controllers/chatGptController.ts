...
export const callChatGpt = async (text: string): Promise<string> => {
  try {
    const referenceData = {
      ...
      "movement_types":[
        {"enum": "NOL", "name": "Noleggio"},
        {"enum": "COM", "name": "Comodato"},
        {"enum": "MNP", "name": "Movimento non produttivo"}
      ]
    };
    ...
    7. Il gruppo di veicoli (id, mnemonic, description).
    8. Il workflow (id, nome).
    9. Rental location (id, nome).
    10. Il tipo di movimento, sicuramente ti verrà indicato il nome e non l'enum. 
